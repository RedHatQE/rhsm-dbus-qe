...
rx_test_config_object_exists = streams['/ws/testware/monitor'] \
    ...
    .map(lambda scenario: [scenario, [verify_dbus_object_exists('/com/redhat/RHSM1/Config')(scenario.events)]])\
    .map(lambda x: TestRun("Config object exists in DBus system tree",
                           scenario = x[0],
