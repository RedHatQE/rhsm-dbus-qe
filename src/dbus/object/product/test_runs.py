...
rx_test_product_object_exists = streams['/ws/testware/monitor'] \
    ...
    .map(lambda scenario: [scenario, [verify_dbus_object_exists('/com/redhat/RHSM1/Product')(scenario.events)]])\
    .map(lambda x: TestRun("Product object exists in DBus system tree",
                           scenario = x[0],
